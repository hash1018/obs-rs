...
**What is monitored** is per source, in three states rather than two: `Off`, `MonitorOnly`, and `MonitorAndOutput`. Heard and recorded are independent questions. Your own microphone is the clearest case — recorded, certainly, and heard back forty milliseconds late is close to unusable to speak over.

That makes two different sums, so there are two mixers. `MonitorOnly` is a source in one and not the other, which no amount of branching off a single mix can express. Both run from startup whether or not anything is set: summing nothing is cheap, and a mixer built later is one every source would have to be registered with again.
...
**A Source that carries its own sound cannot afford that.** Reopening a media file restarts it from the beginning, which is a large thing to do to somebody who only asked to hear it. So its `Tee` is dynamic and `sound::SoundRouting` puts the two mixes on and takes them off while the clip plays: `MonitorOnly` detaches the recording's branch and attaches the monitor's, and going back attaches a fresh one. Each side is answered separately and recorded separately, so one failing leaves the other where it got to rather than leaving the routing lying about both.

That is also what a live stream's sound goes through, though nothing yet gives one a mode: it answers `Off`, which is what puts its branch on the recording's mix and has always been where it went.
