...
A source's `Tee` therefore has up to three branches — the meter, the recording's mix, the monitor mix — and which of them it has is settled when the `Tee` is built. A `Tee`'s branches are fixed at construction, so changing a mode reopens that one capture, the same cost and the same shape as changing its device. The monitor branch registers under a name of its own (`audio-1-monitor`), which buys nothing at run time and everything in a log: two branches into two elements with one name between them cannot be told apart in a topology diagram.
