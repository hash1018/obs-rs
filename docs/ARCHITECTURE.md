...
There is no meter behind a media file's channel yet. Levels are measured on the audio thread and a file's audio runs on the video one, so a meter needs one place both can publish into. `AudioSourceSnapshot::peak_db` was always an `Option` for this shape of reason, and an unmeasured channel draws the empty gauge rather than nothing.
