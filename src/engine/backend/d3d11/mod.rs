...
use crate::engine::source::scene::SceneRegistry;
    ...
    pub(in crate::engine) fn start(
        ...
        let (compositor, handle) = D3d11VideoCompositor::new(
            ...
            VideoCompositorOptions {
                ...
                // The Canvas is the whole of what is seen, so it is opaque.
...
impl RunningSource {
    ...
    ///
    /// For a share of a capture other items also draw from, only this
    /// item's branch of it — see [`CaptureRegistry::stats`]. `None` once
    /// the capture has gone.
