...
//! # The bridge
//!
//! Both filters take `f32`, and noise suppression takes 48 kHz only. A
...
//! nothing: neither element has a switch, and one that is not there costs
//! nothing at all. So enabling, adding, removing and reordering all refill
//! the rack, and only a gate's settings go through a handle.
...
use media_pp::elements::{
    AudioFormat, AudioGate, AudioGateHandle, AudioGateOptions, AudioResampler,
    NOISE_SUPPRESSOR_SAMPLE_RATE, NoiseSuppressor, Rack, RackHandle,
};
use media_pp::ffmpeg;
...
/// What a build answers: the elements for the rack, in order, and the gates
/// among them by the filter they stand for — two lists of different lengths,
/// since a bridge and a suppressor have nothing to retune.
type Built = Result<
    (
        Vec<Box<dyn PpFilter>>,
        HashMap<AudioFilterId, AudioGateHandle>,
    ),
    BackendError,
>;
...
pub(super) struct AudioFilterRack {
    ...
    /// The gates among them, for settings that change without a refill.
    gates: HashMap<AudioFilterId, AudioGateHandle>,
}
...
impl AudioFilterRack {
    ...
    /// otherwise the gates are retuned through their handles and nothing is
    ...
    pub(super) fn apply(&mut self, filters: &[AudioFilter]) -> Result<(), BackendError> {
        ...
        let (elements, gates) = self.build(filters)?;
        self.handle
            .replace(elements)
    ...
    pub(super) fn retune(&self, id: AudioFilterId, settings: &AudioFilterSettings) {
        if let (Some(gate), AudioFilterSettings::NoiseGate(settings)) =
            (self.gates.get(&id), settings)
        {
            // Put in order first, so this cannot be refused.
            let _ = gate.set_options(gate_options(*settings));
        }
    }
        ...
        for filter in on {
            match filter.settings {
                AudioFilterSettings::NoiseSuppression => {
                    elements.push(Box::new(NoiseSuppressor::new(format!(
                        "{}-denoise-{}",
                        self.name, filter.id.0
                    ))));
                }
                AudioFilterSettings::NoiseGate(settings) => {
                    let (gate, handle) = AudioGate::with_options(
                        format!("{}-gate-{}", self.name, filter.id.0),
                        gate_options(settings),
                    )
                    .map_err(|error| BackendError::from(error.to_string()))?;
                    elements.push(Box::new(gate));
                    gates.insert(filter.id, handle);
                }
            }
        }
        Ok((elements, gates))
